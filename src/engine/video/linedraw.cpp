...
void DrawHLine(Uint32 color, int x, int y, int width)
{
	GLubyte r, g, b, a;
	Video.GetRGBA(color, NULL, &r, &g, &b, &a);
	GLint vertices[] = { x, y, x + width, y };
	GLubyte colors[] = {r, g, b, a, r, g, b, a};
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_COLOR_ARRAY);
...
void DrawLine(Uint32 color, int x1, int y1, int x2, int y2)
{
	...
	Video.GetRGBA(color, NULL, &r, &g, &b, &a);
	GLint vertices[] = { xx1, yy1, xx2, yy2 };
	GLubyte colors[] = {r, g, b, a, r, g, b, a};
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_COLOR_ARRAY);
	glVertexPointer(2, GL_INT, 0, vertices);
...
**      clip rectangle, used for faster determination of clipped position.
**
...
void DrawLineClip(Uint32 color, int x1, int y1, int x2, int y2)
{
	...
	//        position(s) on the clipped rectangle should be denoted to the line
	//        drawing routine..
...
void DrawRectangle(Uint32 color, int x, int y, int w, int h)
{
	...
	Video.GetRGBA(color, NULL, &r, &g, &b, &a);
	GLint vertices[] = { x, y, x, y + h, x + w, y + h, x + w, y };
	GLubyte colors[] = {r, g, b, a, r, g, b, a, r, g, b, a, r, g, b, a};
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_COLOR_ARRAY);
	glVertexPointer(2, GL_INT, 0, vertices);
	glColorPointer(4, GL_UNSIGNED_BYTE, 0, colors);
	...
	int w, int h)
{
	GLubyte r, g, b, a;
	Video.GetRGBA(color, NULL, &r, &g, &b, &a);
	GLint vertices[] = { x, y, x, y + h, x + w, y, x + w, y + h };
	GLubyte colors[] = {r, g, b, a, r, g, b, a, r, g, b, a, r, g, b, a};
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_COLOR_ARRAY);
	glVertexPointer(2, GL_INT, 0, vertices);
