...
#ifdef __PLAYBOOK__
	speed = 2;
	printf("Scroll map x=%d y=%d srcX=%d srcY=%d\n", x, y, CursorStartX, CursorStartY);
	int diffX = x - CursorStartX;
	int diffY = y - CursorStartY;
#endif
	UI.MouseViewport->Set(UI.MouseViewport->MapX, UI.MouseViewport->MapY,
		UI.MouseViewport->OffsetX - speed * diffX,
		UI.MouseViewport->OffsetY - speed * diffY);
#ifdef __PLAYBOOK__
	CursorStartX = x;
	CursorStartY = y;
#else
...
/**
**  Detect long selection click, FIXME: temporary hack to test the feature.
*/
#define LongSelected (MouseButtons & ((LeftButton << MouseHoldShift)))
...
#ifdef __PLAYBOOK__
			// move map mode
			SDL_GetMultiMouseState(0, &CursorStartX, &CursorStartY);
			//CursorStartX = CursorX;
			//CursorStartY = CursorY;
			const int TAP_AREA = 5;
			int num = SelectUnitsInRectangle(
					CursorStartX - TAP_AREA, CursorStartY - TAP_AREA,
					CursorStartX + TAP_AREA, CursorStartY + TAP_AREA);
			if (num > 1)
				printf("selected %d units\n", num);
			else
				GameCursor = UI.Scroll.Cursor;
#else
			// disable rectangular unit selection in tablet mode
			CursorStartX = CursorX;
			CursorStartY = CursorY;
	...
	//
		...
		if (MouseButtons & LeftButton) { // enter move mini-mode
			// temporary disable minimap scrolling because of bugs
			/*
			UI.SelectedViewport->Center(
				UI.Minimap.Screen2MapX(CursorX), UI.Minimap.Screen2MapY(CursorY),
				TileSizeX / 2, TileSizeY / 2);
